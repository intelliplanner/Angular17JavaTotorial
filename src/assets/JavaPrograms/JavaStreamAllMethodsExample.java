/**
 * Program: JavaStreamAllMethodsExample 
 */
...
//		findUnMatchedStringfromList();
	}
	
	private static void departmentwiseHighestSalary() {
		List< Java8.Stream.Employee> li =new ArrayList<>();
		li.add(new Java8.Stream.Employee(1, "vicky", "Dev", 1200));
		li.add(new Java8.Stream.Employee(3, "Sanu", "Hr", 1300));
		li.add(new Java8.Stream.Employee(4, "Akan", "Hr", 1100));
		li.add(new Java8.Stream.Employee(2, "Rick", "Dev",1500));
		
		Map<String,Optional<Java8.Stream.Employee>> hs = li.stream()
				.collect(Collectors.groupingBy(Java8.Stream.Employee::getDeparment
						,Collectors.maxBy(Comparator.comparingInt(Java8.Stream.Employee::getSalary))));
...
//				.peek(i->System.out.println(i))
				.mapToObj(i->new Employee(i, 99*i, "Emp_"+i)).collect(Collectors.toList());
...
//		System.out.println(list2);
	...
	private static void findUnMatchedStringfromList(){
		List<String> l1 = Arrays.asList("ABC","BCD","PNC");
		List<String> l2 = Arrays.asList("ABC","BCD","BNT");
		List<String> m1 = Stream.concat(l1.stream(),l2.stream()).distinct().collect(Collectors.toList());
		List<String> m2 = l1.stream().filter(p->l2.contains(p)).collect(Collectors.toList());
		System.out.println(m1 +"\n"+ m2);
		m1.removeAll(m2);
		System.out.println(m1);
//		m.stream().filter(p-> m.contains(p)).collect(Collectors.toList()).forEach(System.out::println);
